...
import { PromptItem, Category, ChatSession, PromptDocument } from './types';
...
export const Sidebar: React.FC<SidebarProps> = ({
    ...
    if (searchQuery.trim()) {
      ...
      return list.map(p => {
        ...
        p.tags.forEach(tag => {
           ...
           if (t === q) score += 3000;
           else if (t.includes(q)) score += 800;
        });
        if (p.prompt.toLowerCase().includes(q)) score += 100;
        return { p, score };
      })
  ...
  return (
    <div 
      className={`flex flex-col border-r border-[var(--border)] bg-[var(--bg-panel)]/50 relative shrink-0 ${isResizing ? 'transition-none' : 'transition-all duration-300'} ${selectedPromptId || selectedDocId ? 'hidden lg:flex' : 'flex'}`}
            ...
            {currentView === 'history' && (
              <div className="space-y-3 animate-in fade-in duration-300">
                <div className="relative group">
                  ...
                  <input type="text" placeholder="Search archive..." className="w-full pl-9 pr-4 py-2 rounded-xl bg-[var(--bg-element)] border border-[var(--border)] focus:outline-none focus:ring-2 focus:ring-[var(--accent)] text-sm" value={historySearchQuery} onChange={(e) => setHistorySearchQuery(e.target.value)} />
                </div>
                <div className="flex gap-2 p-1 bg-[var(--bg-element)]/50 rounded-lg">
                   {(['recent', 'name', 'model'] as const).map(s => (
                     <button key={s} onClick={() => setHistorySortOrder(s)} className={`flex-1 py-1 rounded-md text-[9px] font-bold uppercase transition-all ${historySortOrder === s ? 'bg-[var(--accent)] text-white' : 'text-[var(--text-muted)] hover:text-[var(--text-body)]'}`}>{s}</button>
                   ))}
                </div>
                <div className="space-y-2">
                  {filteredHistory.map(session => (
                    <div key={session.id} onClick={() => onResumeSession(session)} className="group p-3 rounded-xl bg-[var(--bg-element)] border border-[var(--border)] hover:border-[var(--accent)] cursor-pointer transition-all relative">
                      ...
                      <div className="flex items-center gap-2 mt-2">
                         <span className="text-[9px] font-mono text-[var(--text-muted)] bg-[var(--bg-panel)] px-1.5 py-0.5 rounded">{session.modelId}</span>
